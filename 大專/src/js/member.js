...
$(document).ready(function(){
    ...
    $('.registerPassword').on('input',function(){
        if(length_check.test(pwd.value) && upper_check.test(pwd.value) && lower_check.test(pwd.value) && number_check.test(pwd.value)){
            ...
            if(pwd.value.length > 15){
                ...
                $('.passwordAlertCharacters').html('Weak password').css('color','#ff2828');
            }
            pwdFlag = 1;
        }
        else{
            pwdFlag = 0;
            $('.passwordAlertCharacters').html('Enter at least 8 characters').css('color','#ff2828');
            $('.passwordAlertLower').css('color','#ff2828');
            $('.passwordAlertUpper').css('color','#ff2828');
            $('.passwordAlertNum').css('color','#ff2828');
    ...
    $('.registerEmail').on('input',function(){
        if(email_check.test($('.registerEmail')[0].value)){
            ...
            $('.emailAlert').html('Email address is invalid!').css('color','#ff2828').css('display','block');
            emailFlag = 0;
        }
    })
    ...
    $('.registerName').on('input',function(){
        if($('.registerName')[0].value.length < 1){
            $('.usernameAlert').html('Please enter user name!').css('color','#ff2828').css('display','block');
            nameFlag = 0;
        }else{
